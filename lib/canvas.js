...
export default function create(canvasUrl, canvasToken) {
    ...
    return {
        ...
        deleteDeveloperKeyById: async (developerKeyId) => {
            ...
            try {
                const response = await requestJson({
                    url: `${canvasUrl}${CANVAS_DELETE_DEV_KEY_API_URL}${developerKeyId}`,
                    method: 'DELETE',
                    headers: REQUEST_CONFIG.headers
                })
        ...
        updateLtiDeveloperKey: async (developerKeyId, developerKeyBody) => {
            ...
            try {
                const response = await requestJson({
                    url: `${canvasUrl}${updateLtiToolUrl}`,
                    method: 'PUT',
                    headers: REQUEST_CONFIG.headers,
                    body: developerKeyBody
                })
                return response.data
            } catch (error) {
                checkError(error)
        ...
        updateApiDeveloperKey: async (developerKeyId, developerKeyBody) => {
            ...
            try {
                const response = await requestJson({
                    url: `${canvasUrl}${updateApiToolUrl}`,
                    method: 'PUT',
                    headers: REQUEST_CONFIG.headers,
                    body: developerKeyBody
                })
        ...
        createLtiDeveloperKey: async (developerKeyBody) => {
            ...
            try {
                const response = await requestJson({
                    url: `${canvasUrl}${CANVAS_CREATE_LTI_KEY_API_URL}`,
                    method: 'POST',
                    headers: REQUEST_CONFIG.headers,
                    body: developerKeyBody
                })
        ...
        createApiDeveloperKey: async (developerKeyBody) => {
            ...
            try {
                const response = await requestJson({
                    url: `${canvasUrl}${CANVAS_CREATE_API_KEY_API_URL}`,
                    method: 'POST',
                    headers: REQUEST_CONFIG.headers,
                    body: developerKeyBody
                })
        ...
        enableDeveloperKey: async (developerKeyId) => {
            ...
            try {
                const response = await requestJson({
                    url: `${canvasUrl}${enableDevKeyApiUrl}`,
                    method: 'POST',
                    headers: REQUEST_CONFIG.headers,
                    body: enableDevKeyBodyObject
                })
                return response.data
            } catch (error) {
                checkError(error)
